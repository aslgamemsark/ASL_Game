...
This skill turns the project's sign rules (CLAUDE.md, `.claude/rules/fixes.md`) into an executable,
...
Before starting, read (if not already in context this session): `CLAUDE.md` (the NON-NEGOTIABLE
rule + ARCHITECTURE PATTERN sections), `.claude/rules/fixes.md`, and the target scenario's sign
...
STOP and push back — ask for the movement spec before writing anything. (CLAUDE.md, verbatim.)
...
- If no single threshold separates correct from confusor on a parameter, the parameter choice or
  ...
  pass. (`.claude/rules/fixes.md`: threshold-widening as a substitute for a root-cause fix is
...
- If a threshold was a judgment call, add a one-line comment at its use site explaining the
  confusor it guards against (comments live in the code, per `.claude/rules/comments.md`, not the
  commit message).
