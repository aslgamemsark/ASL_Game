...
/**
 * Dev-only overlay showing the ML veto layer's live decisions — rule prediction vs AI vote vs
 * final pass/veto — so you can watch the gate work without opening the console. Renders nothing
 * outside `import.meta.env.DEV` (statically eliminated from the production bundle by Vite, same
 * pattern as CLASSIFIER_DEBUG in config/classifier.ts).
 */
export function ClassifierDevPanel({ status, lastVote }: Props) {
  ...
  return (
    <div className="fixed bottom-2 right-2 z-[9999] font-mono text-[11px] max-w-xs">
      ...
      >
        <span className="font-bold text-purple-300">🤖 AI veto ({status})</span>
        <span>{collapsed ? '▲' : '▼'}</span>
      </button>
      {!collapsed && (
        <div className="bg-black/85 text-white px-2.5 py-2 rounded-b-lg border border-t-0 border-purple-500/50 flex flex-col gap-1">
          {!lastVote && <p className="text-gray-400">No vote yet — pass a sign to see gate output.</p>}
          {lastVote && (
            <>
