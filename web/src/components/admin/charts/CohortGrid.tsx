import { maxRetentionOffset, retentionCell, type RetentionCohort } from '@/lib/adminAnalytics';
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full border-separate border-spacing-1 text-xs">
        <thead>
          <tr>
            <th className="text-left font-semibold text-z-gray-400 px-1 whitespace-nowrap">Signed up</th>
            <th className="text-right font-semibold text-z-gray-400 px-1">Users</th>
            {offsets.map((o) => (
              <th key={o} className="font-semibold text-z-gray-400 px-1 whitespace-nowrap">W{o}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {cohorts.map((c) => (
            <tr key={c.cohort_week}>
              <td className="text-z-gray-200 px-1 whitespace-nowrap">{c.cohort_week.slice(5)}</td>
