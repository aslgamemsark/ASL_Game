...
export function Scoreboard({ entries }: Props) {
  return (
    <div role="list" aria-label="Scores" className="flex items-center gap-2 bg-z-card border border-white/8 rounded-2xl px-3 py-2 overflow-x-auto">
      {entries.map((e, i) => (
        <div role="listitem" key={i} className="flex items-center gap-1.5 shrink-0">
