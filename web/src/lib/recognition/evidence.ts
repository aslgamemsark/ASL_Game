...
export function measureRecognitionEvidence(frames: readonly Frame[], sign: Sign): RecognitionEvidence {
  if (frames.length === 0) {
    return { requiredHandCoverage: 0, clippedFrameRatio: 0, poseCoverage: 0, signerScale: 0, durationSeconds: 0, maxFrameGapSeconds: 0, normalizedWristMotion: 0 };
  }

  const requiredHands = sign.twoHanded ? 2 : 1;
  const signerScales = frames.map((frame) => {
    const width = frameShoulderWidth(frame);
    return width === null || frame.width <= 0 ? null : width / frame.width;
  ...
  const normalizedMotion = frames.slice(1).map((frame, index) => {
    ...
    if (!width || !frame.hands[0] || !before.hands[0]) return null;
  ...
  }).filter((motion): motion is number => motion !== null);

  return {
    requiredHandCoverage: handCoverage,
    clippedFrameRatio,
    poseCoverage,
    signerScale: signerScales.length === 0 ? 0 : signerScales.reduce((sum, scale) => sum + scale, 0) / signerScales.length,
    durationSeconds: Math.max(0, frames.at(-1)!.t - frames[0].t),
    maxFrameGapSeconds: gaps.length === 0 ? 0 : Math.max(...gaps),
    normalizedWristMotion: normalizedMotion.length === 0 ? 0 : normalizedMotion.reduce((sum, motion) => sum + motion, 0) / normalizedMotion.length,
  };
}
